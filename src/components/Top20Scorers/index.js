  ...
  useEffect(() => {
    ...
    if (!!leagueId) {
      if (leagueId === PREMIER_LEAGUE_ID) {
        //f user has the id 2021 as the league id in firebase,load the top 20 premier league data from PremierLeagueTop20.js
        setTopScorersArr(TOP_20_PREMIER_LEAGUE[0].response);
      } else if (leagueId === CHAMPIONS_LEAGUE_ID) {
        //if user has the id 2001 as the league id in firebase,load the top 20 champions league data from UEFAChampionsLeagueTop20.js
        ...
        <tbody>
          {props.topScorersArr.map((item, index) => (
            <tr key={index}>
              ...
              <td>
                <img src={item.player.photo} alt="player photo" />
              </td>
              <td>{item.statistics[0].team.name}</td>
              <td>
                <img src={item.statistics[0].team.logo} alt="team logo" />
              </td>
...
const Chart = (props) => {
  ...
  useEffect(() => {
    if (props.topScorersArr !== undefined && props.topScorersArr.length > 0) {
      ...
      const no_of_goals_arr = props.topScorersArr.map(
        (item) => item.statistics[0].goals.total
      );
      const no_of_shots_arr = props.topScorersArr.map(
        (item) => item.statistics[0].shots.total
      );
      const imges_arr = props.topScorersArr.map((item) => item.player.photo);
      const no_of_red_card = props.topScorersArr.map(
        (item) => item.statistics[0].cards.red
      );
      const no_of_yellow_card = props.topScorersArr.map(
        (item) => item.statistics[0].cards.yellow
      );
      const no_of_red_yellow_card = props.topScorersArr.map(
        (item) => item.statistics[0].cards.yellowred
      );
